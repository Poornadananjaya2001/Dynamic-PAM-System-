...
class PAMPerformanceTester:
    def __init__(self, base_url="http://127.0.0.1:5000"):
        ...
        self.results = {}
        try:
            import app as pam_app
            self.flask_client = pam_app.app.test_client()
        except Exception:
    ...
    def test_operation_response_times(self, operation_name, url, method="GET", data=None, iterations=50):
        ...
        for i in range(iterations):
            try:
                ...
                # Check live server first, fallback to test client
                if self.flask_client:
                    if method == "GET":
                        response = self.flask_client.get(path)
                    elif method == "POST":
                        response = self.flask_client.post(path, json=data)
                else:
                    if method == "GET":
